...
export default function Verify() {
  ...
  // Poll until parse-cv finishes saving the profile (runs concurrently with animation)
  useEffect(() => {
        ...
        {profileLoading ? (
          ...
          <>
            ...
            <div className="fu" style={{ textAlign:'center', marginBottom:36 }}>
              ...
              <p style={{ fontSize:14, color:'#6b7280' }}>We'll send a code to verify it's you</p>
            </div>
            ...
            {!codeSent ? (
              ...
              <div className="fu d1">
                ...
                />
                {sendError && (
                  <p style={{ fontSize:12, color:'#dc2626', marginBottom:10 }}>{sendError}</p>
                )}
                <button
                  onClick={sendCode}
                  disabled={sending || !email.trim()}
                  style={{ width:'100%', background:'#10b981', color:'#fff', border:'none', borderRadius:10, padding:'13px 0', fontSize:15, fontWeight:700, cursor:'pointer', opacity: (sending || !email.trim()) ? 0.6 : 1, marginTop:4 }}
                >
                  {sending ? 'Sending…' : 'Send verification code →'}
                </button>

                <div className="fu d2" style={{ textAlign:'center', marginTop:20 }}>
                  <button
              ...
              /* ── Code entry ── */
              <div className="fu d1" style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:14, padding:'24px 22px' }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    outline:'none', color:'#111', background:'#fff', textAlign:'center',
                    marginBottom:8,
                  }}
                />
                {codeError && (
                  <p style={{ fontSize:12, color:'#dc2626', marginBottom:10 }}>{codeError}</p>
                )}

                <button
                  onClick={verifyCode}
                  disabled={verifying || code.length < 6}
                  style={{ width:'100%', background:'#10b981', color:'#fff', border:'none', borderRadius:10, padding:'13px 0', fontSize:15, fontWeight:700, cursor:'pointer', opacity: (verifying || code.length < 6) ? 0.6 : 1, marginTop:4 }}
                >
                ...
                <button
                  onClick={sendCode}
                  style={{ width:'100%', marginTop:10, background:'none', border:'none', color:'#9ca3af', fontSize:12, cursor:'pointer' }}
                >
                  Resend code
                </button>
              </div>
